...
Evaluate baseline model on the validation set, which contains the first 10,000 clean images.

```{r}
# Predict on first 10,000 clean images and show results
...
Evaluate the CNN model on the validation set, which contains the first 10,000 clean images.

```{r}
# Predict on first 10,000 clean images and show results
pred1 = model %>%
  predict(cifar$test$x) %>%
  k_argmax() %>%
  as.matrix() %>%
  as.factor()
