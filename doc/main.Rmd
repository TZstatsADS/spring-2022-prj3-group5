...
Here we approached the prediction modelling process in two main steps: First, we will train a CNN that does the label correction (i.e., mapping noisy labels to cleaned labels, conditioned on the input image). Then we use this CNN to correct the noisy labels to get cleaned ones, which will then be used to train our CNN in Model 1.

+ **Step 1**: Train a label correction network on the first 10,000 images with clean labels and predict on the last 40,000 images to correct their labels.
+ **Step 2**: Once the labels have been corrected, we will train a new CNN (similar to Model 1) on the corrected labels. 
